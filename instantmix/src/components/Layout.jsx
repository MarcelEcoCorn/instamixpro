...
const NAV = [
  { id: 'receptury',  label: 'Receptury',          roles: ['admin','technolog','sprzedaz'] },
...
export default function Layout({ page, setPage, children }) {
  ...
  let visibleNav = NAV.filter(n => n.roles.includes(role))
  if (role === 'sprzedaz') {
    visibleNav = SPRZEDAZ_ORDER.map(id => NAV.find(n => n.id === id)).filter(Boolean)
  }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ background:'#fff', borderBottom:'0.5px solid #D3D1C7', display:'flex', padding:'0 16px', overflowX:'auto', position:'sticky', top:'52px', zIndex:199 }}>
        {visibleNav.map(n => (
          <button key={n.id} onClick={() => setPage(n.id)} style={{ padding:'12px 14px', fontSize:'13px', border:'none', background:'none', cursor:'pointer', borderBottom:page===n.id?'2px solid #1D9E75':'2px solid transparent', color:page===n.id?'#0F6E56':'#5F5E5A', fontWeight:page===n.id?'500':'400', whiteSpace:'nowrap', transition:'all .15s', flexShrink:0 }}>
            {n.label}
          </button>
        ))}
      </div>
