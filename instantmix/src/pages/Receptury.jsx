...
export default function Receptury() {
  ...
  const [form, setForm] = useState({ code:'', name:'', version:'v1', status:'w_przegladzie', production_line:'zwykla', approved_at:'', notes:'' })
  ...
  function openNew() {
    setForm({ code:'', name:'', version:'v1', status:'w_przegladzie', production_line:'zwykla', approved_at:'', notes:'' })
  ...
  async function save() {
    ...
    const total = items.reduce((s, it) => s + (parseFloat(it.percentage) || 0), 0)
    if (Math.abs(total - 100) > 0.01) { setError(`Suma udziałów musi wynosić 100% (aktualnie: ${total.toFixed(2)}%)`); return }
    setSaving(true); setError('')
    const { data: rec, error: recErr } = await supabase.from('recipes').insert({
      code: form.code, name: form.name, version: form.version, status: form.status,
      production_line: form.production_line, approved_at: form.approved_at || null,
      notes: form.notes, approved_by: profile?.id
    }).select().single()
    if (recErr) { setError(recErr.message); setSaving(false); return }
    await supabase.from('recipe_items').insert(
      items.filter(it => it.ingredient_id).map((it, idx) => ({
        recipe_id: rec.id, ingredient_id: it.ingredient_id,
        percentage: parseFloat(it.percentage), sort_order: idx
      }))
    )
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div className="flex">
          <input className="search" placeholder="Szukaj receptury..." value={search} onChange={e => setSearch(e.target.value)} style={{ width:220 }} />
      ...
      <div className="card-0">
        <table>
          <thead><tr><th>Kod</th><th>Nazwa mieszanki</th><th>Wersja</th><th>Linia</th><th>Składników</th><th>Status</th><th>Data zatw.</th><th></th></tr></thead>
          <tbody>
            {loading && <tr><td colSpan={8} style={{ textAlign:'center', padding:'24px', color:'#888' }}>Ładowanie...</td></tr>}
            {!loading && filtered.map(r => (
              <tr key={r.id}>
                ...
                <td><span className={`badge ${r.status==='dopuszczona'?'b-ok':r.status==='w_przegladzie'?'b-warn':'b-gray'}`}>{r.status}</span></td>
                <td className="muted">{r.approved_at || '—'}</td>
                <td><button className="btn btn-sm" onClick={() => setDetail(detail?.id === r.id ? null : r)}>Szczegóły</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {detail && (
        <div className="card" style={{ borderLeft:'3px solid #1D9E75' }}>
          <div className="flex" style={{ marginBottom:10, flexWrap:'wrap', gap:6 }}>
            ...
            <button className="btn btn-sm" style={{ marginLeft:'auto' }} onClick={() => setDetail(null)}>Zamknij</button>
          </div>
          <div className="card-0">
            <table>
      ...
      <div className={`modal-overlay ${modal?'open':''}`} onClick={e => e.target===e.currentTarget && setModal(false)}>
        <div className="modal">
          <div className="modal-title">Nowa receptura</div>
          ...
          <div style={{ fontWeight:500, fontSize:13, marginBottom:8 }}>Składniki receptury</div>
          {items.map((it, idx) => (
            <div key={idx} style={{ display:'grid', gridTemplateColumns:'1fr 100px 32px', gap:8, marginBottom:8, alignItems:'end' }}>
              <div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8 }}>
            <button className="btn btn-sm" onClick={addItem}>+ Dodaj składnik</button>
            <span style={{ fontSize:12, color: Math.abs(items.reduce((s,it)=>s+(parseFloat(it.percentage)||0),0)-100)<0.01 ? '#085041' : '#A32D2D' }}>
              Suma: {items.reduce((s,it)=>s+(parseFloat(it.percentage)||0),0).toFixed(3)}%
            </span>
          </div>
          <div className="modal-footer">
            <button className="btn" onClick={() => setModal(false)}>Anuluj</button>
            <button className="btn btn-primary" onClick={save} disabled={saving}>{saving?'Zapisywanie...':'Zapisz recepturę'}</button>
          </div>
        </div>
      </div>
    </div>
